  ...
  return {
    ...
    entries: [
      ...
      },
      {
        ...
        type: 'capability',
        name: 'Agent task (A2A)',
        description: 'Accepts A2A message/send requests and answers type-matchup questions.',
        protocols: ['a2a'],
        url: A2A_ENDPOINT,
      },
    ],
...
export function authorizationServerDocument() {
  return {
    ...
    // auth.md / agent-auth block. Reserved placeholder: every endpoint below returns
    // HTTP 503 until the authorization server is deployed. See AUTH_MD.
    agent_auth: {
      status: 'under_construction',
      under_construction: true,
      skill: SKILL_FILE_URL,
      identity_endpoint: `${SITE}/oauth/agent/identity`,
      claim_endpoint: `${SITE}/oauth/agent/identity/claim`,
      events_endpoint: `${SITE}/oauth/agent/events`,
      identity_types_supported: ['identity_assertion', 'service_auth', 'anonymous'],
      identity_assertion: {
        assertion_types_supported: ['urn:ietf:params:oauth:grant-type:jwt-bearer'],
      },
      events_supported: ['https://schemas.openid.net/secevent/risc/event-type/credential-change'],
      note:
        'Reserved placeholder. These endpoints are not deployed and return HTTP 503. See ' + AUTH_MD_URL + '.',
    },
